...
export default function Reports({ pumps }) {
  ...
  const searchHandler = e => {
    ...
    const filtered = pumps.filter(pump => {
      return pump.latitude.toString().includes(e.target.value.toString()) || pump.longitude.toString().includes(e.target.value.toString()) || pump.village.commune.toLowerCase().includes(e.target.value.toLowerCase()) || pump.village.district.toLowerCase().includes(e.target.value.toLowerCase()) || pump.village.province.toLowerCase().includes(e.target.value.toLowerCase()) || pump.village.village.toLowerCase().includes(e.target.value.toLowerCase()) || pump.well_depth.toString().includes(e.target.value.toString()) || pump.id.toLowerCase().includes(e.target.value.toLowerCase()) || (pump.status === 2 && (e.target.value.toLowerCase() === 'functional')) || (pump.status === 1 && (e.target.value.toLowerCase() === 'unknown')) || (pump.status === 0 && (e.target.value.toLowerCase() === 'non-functional'))
    })
    setSearch(filtered)
  }
